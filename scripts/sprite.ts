...
const inputDir = path.join(cwd, "src", "components", "icons");
...
async function generateIconFiles() {
  ...
  const typeOutputFilepath = path.join(typeDir, "icon.d.ts");
  const utilsDirPath = path.join(utilsDir, "icons.ts");
  ...
  const currentTypes = await fsExtra
    ...
    .catch(() => "");
  await fsExtra.readFile(utilsDirPath, "utf8").catch(() => "");
  ...
  const typeOutputContent = `// This file is generated by bun run icons:generate

export type IconName =
\t| ${stringifiedIconNames.join("\n\t| ")};
`;

  const icons = `// This file is generated by npm run build:icons
    export const icons = [${stringifiedIconNames.join(", ")}];
    `;
  ...
  const utilsChanged = await writeIfChanged(utilsDirPath, icons);
...
`,
  ...
  if (spriteChanged || typesChanged || readmeChanged || utilsChanged) {
