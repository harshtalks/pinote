...
export const tfRouter = createTRPCRouter({
  ...
  register: authenticatedProcedure
    ...
    .mutation(({ ctx, input }) =>
      Effect.gen(function* () {
        ...
        Effect.withSpan("tfRouter.register"),
  ...
  tfStatus: authenticatedProcedure.query(({ ctx }) =>
    ctx.session.pipe(
      ...
      Result.catchAllDefect,
      provideDB,
      Effect.withSpan("tfRouter.tfStatus"),
      trpcRunTime.runPromise,
    ),
  ),
  ...
  tfVerify: authenticatedProcedure
    ...
    .mutation(({ ctx, input }) =>
      Effect.gen(function* () {
        ...
        Result.catchAll,
        Result.catchAllDefect,
        Effect.withSpan("tfRouter.tfVerify"),
        trpcRunTime.runPromise,
      ),
    ),
  ...
  tfSkip: authenticatedProcedure
    ...
    .mutation(({ ctx, input }) =>
      ctx.session.pipe(
        ...
        Result.catchAllDefect,
        provideDB,
        Effect.withSpan("tfRouter.tfSkip"),
        trpcRunTime.runPromise,
      ),
    ),
  // Create a Challenge
  createChallenge: authenticatedProcedure.mutation(() =>
    TfChallenges.pipe(
      ...
      Result.catchAllDefect,
      provideChallengeRef,
      Effect.withSpan("tfRouter.createChallenge"),
      trpcRunTime.runPromise,
    ),
  ),
  updateTfStatus: authenticatedProcedure
    ...
    .mutation(({ ctx, input }) => {
      return ctx.session.pipe(
        ...
        Result.catchAllDefect,
        provideDB,
        Effect.withSpan("tfRouter.updateTfStatus"),
        trpcRunTime.runPromise,
      );
    }),
  ...
  recoverAccount: authenticatedProcedure
    ...
    .mutation(({ ctx, input }) => {
      return Effect.gen(function* () {
        ...
        Result.catchAll,
        Result.catchAllDefect,
        Effect.withSpan("tfRouter.recoverAccount"),
        trpcRunTime.runPromise,
      );
    }),
});
